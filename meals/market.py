...
The repository already has the detector for this. meals.detector forecasts
...
from meals import routing, severity, windows

log = logging.getLogger("meals.market")
...
DEFAULT_EVENTS_PATH = os.path.join("data", "meals", "market_events.parquet")
...
def main(argv: list[str] | None = None) -> int:
    ...
    from meals import detector, versioning
    ...
    if not os.path.exists(args.scores):
        log.error("No detector scores - run python -m meals.detector first")
        return 2
