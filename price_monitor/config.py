...
class Config:
    ...
    daily_cooldown_minutes: int = 2880
    daily_escalation_factor: float = 1.5
    # Muting the alerts while Tremor takes over (see README). The run proceeds as
    ...
    # The same switch for the Tremor side, and DEFAULT ON - that is, silent.
    # The two are separate because they mute different systems: alerts_muted
    # silences the crude per-asset signals that Tremor replaces, and this one
    # silences Tremor itself. Wiring the delivery is not the same act as
