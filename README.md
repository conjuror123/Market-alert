...
прошлых дат — только «эта неделя». Помесячные страницы
(`forexfactory.com/calendar?month=sep.2026`) при этом читаются: `requests`
получает от них 403 при любых заголовках, включая полный браузерный набор, а
`urllib` с тем же User-Agent — 200, потому что различие не в заголовках, а в
TLS-отпечатке клиента. Именно на этом держится дозаполнение факта выше.
Понедельный скрейпинг (`calendar?week=...`) остаётся заблокирован Cloudflare
(проверено вживую: HTTP 403, JS-челлендж "Just a moment..."). Пробовали в качестве платной альтернативы Financial
...
price_monitor/
  ...
  weekly_digest.py — субботний дайджест Medium/High событий календаря в Telegram
