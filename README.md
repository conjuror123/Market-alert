...
4. Запускайте когда захочется: вкладка **Actions → Explain Alerts → Run workflow**.
   Поле **Message ID** позволяет объяснить конкретный алерт — этот номер написан
   мелким текстом в конце самого сообщения в Telegram (строка вида `ID: 12345`).
   Если оставить поле пустым, шаг обработает сразу все ещё не объяснённые алерты.

При запуске бот берёт нужные алерты (один конкретный или все не объяснённые — в
зависимости от поля Message ID), для каждого ищет заголовки новостей по активу
через Google News (ключ не нужен) и отправляет их вместе с цифрами алерта в LLM с
