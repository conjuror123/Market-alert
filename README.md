...
Верхняя граница окна поиска — это всегда момент самого запуска workflow, а не
момент алерта: чем позже после алерта вы нажмёте «Run workflow», тем больше
новостей успеет накопиться. Поэтому у алерта должно пройти не меньше
`explain_min_age_hours` часов (по умолчанию 6) с момента отправки, прежде чем шаг
вообще возьмётся его объяснять — иначе окно поиска могло бы оказаться то в
несколько минут, то в сутки, совершенно случайно, просто в зависимости от того,
когда вы запустили workflow. Алерт младше этого возраста остаётся в очереди и
