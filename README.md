...
4. Настройте автозапуск раз в час — без этого шага мониторинг не будет
   запускаться сам вообще, только вручную через Run workflow. У `price-monitor.yml`
   нет собственного расписания: расписание GitHub Actions ненадёжно на
   малоактивных репозиториях (реально может срабатывать раз в 5-10 часов вместо
   заявленного часа), поэтому вместо него используется внешний, полностью
   бесплатный будильник — см. блок «Надёжный запуск раз в час» ниже, это буквально
   10 минут настройки.
...
- `lookback`/`mad_window` (сейчас 300/288 свечей, ~12 дней) оставлены такими не
  из-за ограничения API — Yahoo отдаёт по 60+ дней за один запрос, а `coinbase.py`
  умеет постранично подгружать сколько угодно свечей мимо лимита Coinbase в 300 за
  запрос. Причина проще: бэктест с базой вплоть до 700 свечей не показал прироста
  ни в частоте, ни в recall при текущих порогах, так что увеличивать эти значения
  пока незачем.
