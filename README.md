...
**Архив хранит только события с `impact: High`**, хотя сам дайджест в Telegram
по-прежнему показывает Medium+High (это про то, что ожидать на неделе вперёд, а
не про целостность архива задним числом). Причина именно такого ограничения — в
следующем разделе.
...
Вместо этого глубокий бэкфилл берёт готовые CSV-дампы ForexFactory с GitHub —
раздаются с `raw.githubusercontent.com`, а это не forexfactory.com, поэтому
Cloudflare туда не дотягивается и ключ не нужен вообще:

- **`github.com/spoluan/forex-factory-scraper`** (MIT, по файлу на год,
  2010–2023; 2024+ проверены вживую — 404, дальше данных нет) — все уровни
  `impact`. Времена в этом CSV — **не UTC и не US Eastern**, а фиксированное
  ...
  времени суток — они приводятся к полуночи UTC той же календарной даты). У
  этого скрейпера также своя, более простая исходная шкала `impact` —
  `Low/Medium/High/Non-economic`, без отдельной метки `Holiday` (праздники
  приходят как `Low`) — не путать с исходной шкалой живого фида ForexFactory
  выше. И то и другое приводится к единой шкале `Low/Medium/High` в самом
  коде (`_normalize_impact` в `economic_calendar.py`): `Holiday` живого фида
  и `Non-economic` этого скрейпера оба схлопываются в `Low` ещё на этапе
  разбора, до любой фильтрации и сохранения — дальше по коду сторонние
  категории каждого источника нигде не всплывают.
- **`github.com/ehsanrs2/forexfactory-scraper`** (GPLv3, один файл
  ...
  impact**, но зато с явным ISO8601-смещением (Europe/London, GMT/BST) прямо
  в каждой дате — в отличие от spoluan, никакой ручной коррекции не нужно,
  обычного `fromisoformat` достаточно (тоже проверено вживую по FOMC Statement
  в разные сезоны 2024–2026, смещение конвертируется в реальное UTC точно).

**Почему в архиве остаётся только High impact**, хотя spoluan даёт и
Medium/Low: полное покрытие есть только для 2010–2023, а после — либо вообще
ничего (до находки ehsanrs2), либо только High. Хранить Medium/Low там, где
они есть, и не хранить там, где их взять неоткуда, значило бы, что «в этом
периоде не нашлось Medium-событий» может означать две разные вещи в
зависимости от даты — вводит в заблуждение при ручном ревью. Поэтому
`filter_high_impact_only` в `economic_calendar.py` применяется к любому
источнику при записи в архив, и весь `data/economic_calendar/calendar.ndjson`
целиком — только High impact, единообразно на любую дату.

Разовый импорт всего сразу:

```bash
python -m price_monitor.economic_calendar --import-spoluan --since-year 2021 --until-year 2023 --import-ehsan-high-impact
```

(или **Actions → Backfill Economic Calendar → Run workflow**, поля
`since_year`/`until_year`/`ehsan_high_impact`) — 2021 выбран нижней границей
для spoluan, потому что именно с этого года начинается локальная история
свечей (`data/candle_history/`, см. выше), более ранние календарные данные
пока не нужны. `--import-ehsan-high-impact` не принимает диапазон дат — это
всегда один файл целиком.

Даже после обоих источников остаётся дыра: **Medium/Low за 2024 — январь
2026 её не закрывают** (архив всё равно хранит только High), и **всё
подряд с февраля по конец августа 2026** — момент, когда собственный
живой фид ForexFactory начал накапливаться сам (см. выше). Других
бесплатных источников без ключа под это не нашлось — проверялись
дополнительно Wayback Machine (недоступна из этого окружения технически),
несколько других GitHub-скрейперов (без закоммиченных данных) и
investing.com/TradingView (без подтверждённого рабочего бесплатного API).
...
.github/workflows/
  ...
  weekly-digest-test.yml — ручная отправка дайджеста календаря прямо сейчас
                          ...
                          воскресенья — не трогает "уже отправлено на этой
                          неделе" в state.json
  ...
  backfill-calendar.yml — ручной импорт data/economic_calendar/ из готовых
                          CSV-дампов ForexFactory на GitHub (spoluan 2010-2023
                          MIT + ehsanrs2 ~2020-2026 High-only GPLv3) — ключ не
                          нужен, raw.githubusercontent.com не за Cloudflare
