...
новости» ниже), только применённая к историческим датам. Результат — читаемый
markdown-отчёт (`data/calibration_review/daily_signal_review.md`) для ручного
просмотра: по нему уже можно судить, стоит ли двигать порог вверх или вниз, глядя на
то, какие реальные новости попали в пойманные и какие — в пропущенные. Инструмент
сигнал-агностичный (работает и с `--signal hourly`), но пока запускался только для
дневного сигнала.
