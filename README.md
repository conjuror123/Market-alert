...
При запуске бот берёт все ещё не объяснённые алерты, для каждого ищет свежие
заголовки новостей по активу через Google News (ключ не нужен), отправляет их вместе
с цифрами алерта в LLM и просит связать новость с движением цены в 2-3 предложениях
на русском. Если подходящей новости не нашлось, модели явно велено так и написать,
а не придумывать причину. Готовое объяснение дописывается в конец исходного
сообщения в Telegram через редактирование поста — новое уведомление не создаётся.
