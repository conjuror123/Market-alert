# Departures from the Tremor 5.1 specification
...
computed explicitly (`tremor.fred.available_at`). Without that the backtest would
...
ones on the round UTC hour boundary (`tremor.bars.to_hourly`).
...
versioned, while the export itself goes to `data/tremor/events/`; that directory and
`data/tremor/residuals/` are in `.gitignore`.
...
**What it costs:** after a clone you have to run `python -m tremor.saed` and
`python -m tremor.export`, or the decision journal will be assembled without SAED rows
...
in `data/tremor/calibration.json` and the freeze in `data/tremor/frozen.json`.
...
**The ladder** (`tremor.severity`). Severity is a **return period**: how long you would
...
`tremor.market`, and it re-scores nothing: `tremor.detector` already forecasts basket
...
**Delivery** (`tremor.routing`). Rarity and urgency are kept apart on purpose: rarity
...
Retention (`tremor.persistence`) is the cumulative abnormal return over the next *h*
...
`tremor.backfill --fill-gaps` re-asks the provider for each missing session individually.
...
**What was NOT done.** `tremor.calibrate` was not run. It writes `frozen.json`, and
...
| alerts/year | budget | Tremor caught | precision | SPY trailing caught | precision |
|---:|---:|---:|---:|---:|---:|
