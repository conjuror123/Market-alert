...
Tremor v1 is finished, calibrated, and tested twice. It reaches F1 23.0% against a
...
Tremor makes breadth (cluster shift) and single-factorness (PCA, correlation) its two
...
**Phase 8.1 `tremor/features.py`.** A feature matrix on the reference-hour grid:
...
**Phase 8.2 `tremor/forecast.py`.** Predict realized basket volatility over the next 24
...
**Phase 8.3 `tremor/threshold.py`.** Turn the forecast into alerts with a time-varying
...
freeze record goes in `data/tremor/frozen.json` before the test is read, as it did for
both v1 tests.
