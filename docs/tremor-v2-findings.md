# Tremor v2 — what the rebuild found
...
What is built and working: `tremor/features.py` (HAR ladder, downside volatility, the
strong signals as levels, everything also standardised), `tremor/forecast.py` (walk-forward
ridge HAR with an honest resolved-target rule), `tremor/threshold.py` (adaptive and fixed
cuts, plus the labels-free firing-rate check), `tremor/detector.py` (the chain end to end).
